...
@NamedQueries({
		...
		@NamedQuery(name = "Transferencia.porAnoAgrupadoPorMes", query = "SELECT t.mes, sum(t.valor) from Transferencia t WHERE t.ano = :ano AND t.municipio.nome <> 'Governo do Estado' AND t.municipio.estado.sigla NOT IN ('OM', 'EX') GROUP BY t.mes", hints = { @QueryHint(name = "org.hibernate.cacheable", value = "true") }),
		@NamedQuery(name = "Transferencia.porAnoAgrupadoPorArea", query = "SELECT t.area.nome, sum(t.valor) from Transferencia t WHERE t.ano = :ano AND t.municipio.nome <> 'Governo do Estado' AND t.municipio.estado.sigla NOT IN ('OM', 'EX') GROUP BY t.area", hints = { @QueryHint(name = "org.hibernate.cacheable", value = "true") }),
		@NamedQuery(name = "Transferencia.porAnoMesAgrupadoPorArea", query = "SELECT t.area.nome, sum(t.valor) from Transferencia t WHERE t.ano = :ano AND t.mes = :mes AND t.municipio.nome <> 'Governo do Estado' AND t.municipio.estado.sigla NOT IN ('OM', 'EX') GROUP BY t.area", hints = { @QueryHint(name = "org.hibernate.cacheable", value = "true") }),
